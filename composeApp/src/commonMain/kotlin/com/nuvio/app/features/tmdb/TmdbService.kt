...
object TmdbService {
    ...
    suspend fun ensureTmdbId(videoId: String, mediaType: String): String? {
        ...
        val normalized = videoId.removePrefix("tmdb:").removePrefix("movie:").removePrefix("series:").substringBefore(':').substringBefore('/').trim()
    ...
    suspend fun tmdbToImdb(tmdbId: Int, mediaType: String): String? {
        ...
        cacheMutex.withLock { tmdbToImdbCache[cacheKey]?.let { return it } }
    ...
    private suspend fun imdbToTmdb(imdbId: String, mediaType: String, apiKey: String): String? {
        ...
        cacheMutex.withLock { imdbToTmdbCache[cacheKey]?.let { return it } }
        val body = fetch<TmdbFindResponse>(endpoint = "find/$imdbId", apiKey = apiKey, query = mapOf("external_source" to "imdb_id")) ?: return null
        ...
        } else log.d { "No TMDB ID found for $imdbId ($normalizedType)" }
        return resultId
    }

    private suspend inline fun <reified T> fetch(endpoint: String, apiKey: String, query: Map<String, String> = emptyMap()): T? {
        val url = buildTmdbUrl(endpoint = endpoint, apiKey = apiKey, query = query)
        return runCatching { json.decodeFromString<T>(httpGetText(url)) }
            .onFailure { error -> log.w { "TMDB request failed for $endpoint: ${error.message}" } }
            .getOrNull()
    }

    private fun currentApiKey(): String? = TmdbSettingsRepository.snapshot().apiKey.trim().takeIf(String::isNotBlank)

    internal fun normalizeMediaType(mediaType: String): String = when (mediaType.trim().lowercase()) {
        "movie", "film" -> "movie"
...
internal fun buildTmdbUrl(endpoint: String, apiKey: String, query: Map<String, String> = emptyMap()): String {
    val params = linkedMapOf("endpoint" to endpoint.removePrefix("/"))
    query.forEach { (key, value) -> if (value.isNotBlank()) params[key] = value }
    return buildString {
        append("https://evzcfcfnnzxullpbinpi.supabase.co/functions/v1/pixbar-tmdb?")
        append(params.entries.joinToString("&") { (key, value) ->
            val encoded = value.replace("%", "%25").replace(" ", "%20").replace("/", "%2F").replace("?", "%3F").replace("&", "%26").replace("=", "%3D")
            "$key=$encoded"
        })
    }
}
...
@Serializable
private data class TmdbExternalResult(val id: Int)

@Serializable
private data class TmdbExternalIdsResponse(@SerialName("imdb_id") val imdbId: String? = null)
